...
- **OpenSpec schema asset**: `packages/pi-mimir/openspec/schemas/review-gated/schema.yaml` must be rewritten to use OpenSpec's supported schema fields and artifact array structure.
- **Schema templates**: `packages/pi-mimir/openspec/schemas/review-gated/templates/{proposal,spec,design,tasks}.md` may be refined as skeletons/examples, but should not carry orchestration policy.
