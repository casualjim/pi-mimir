...
The active Pi session file is available to child commands as `$PI_SESSION_FILE`. To inspect earlier sessions for this same working directory, use the `recall` tool's session search rather than guessing a path or scanning another project.
