...
export const REVIEW_MANAGED_MANIFEST = 'pi-review-managed.json';
...
function manifestPath(): string {
  return join(getAgentRoot(), REVIEW_MANAGED_MANIFEST);
}
...
function bundledAgents(): Manifest {
  ...
  const bundled = ['implementation-reviewer.md'];
  for (const name of bundled) {
    const source = join(BUNDLED_AGENTS_DIR, name);
    try {
      agents[name] = sha256(readFileSync(source));
    } catch {
      // Missing bundled agent becomes packaging/test failure; sync degrades silently at runtime.
    }
  }
  return agents;
}
...
export function syncBundledReviewAgents(): SyncResult {
  ...
  for (const [name, sourceHash] of Object.entries(bundled)) {
    ...
    // Existing unknown or user-edited agents stay user-owned.
  }
