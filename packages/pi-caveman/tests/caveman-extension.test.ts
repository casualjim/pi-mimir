...
import {
  ...
  readModeState,
  safeWriteMode,
} from "../extensions/caveman/mode-state.js";
...
describe("Pi Caveman extension hooks", () => {
  ...
  it("tracks skill and natural-language mode changes before each agent turn", async () => {
    ...
    expect(readModeState()).toBeNull();
    expect(
      await harness.emit("before_agent_start", { systemPrompt: "base prompt" }),
    ).toBeUndefined();

    await harness.emit("input", {
      text: "please use caveman mode",
      source: "interactive",
    });
    expect(readModeState()).toBe("full");
  });
