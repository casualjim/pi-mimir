...
export function buildWorktreeOpenArgs(
	name: string,
	cwd: string,
	path: string,
): string[] {
...
export function buildWtCreateArgs(branch: string, base?: string): string[] {
	return [
		"switch",
		"--create",
		"--no-cd",
...
export function parseWtSwitchOutput(output: string): string {
	for (const line of output.split("\n")) {
		const trimmed = line.trim();
		if (!trimmed.startsWith("{")) continue;
		try {
			// SAFETY: JSON.parse returns unknown; we only probe the `path` key and
			// re-validate with isString before use, so the assertion narrows safely.
			const parsed = JSON.parse(trimmed) as { path?: unknown };
			if (isString(parsed.path) && parsed.path) return parsed.path;
		} catch {
			continue;
		}
	}
	throw new Error(`Unexpected wt switch output: ${output.trim() || "(empty)"}`);
}

function createWtCheckout(cwd: string, branch: string, base: string): string {
	const output = execFileSync(
		"wt",
		buildWtCreateArgs(branch, base === "HEAD" ? undefined : base),
		{ cwd, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] },
	);
	return parseWtSwitchOutput(output);
}
...
export function createHerdrWorktree(
	...
	let output: string;
	if (hasCommand("wt")) {
		// wt owns checkout creation so worktrees get jj support and init steps
		// (mise install, env generation, copy-ignored) from .config/wt.toml.
		const checkoutPath = createWtCheckout(cwd, branch, base);
		output = herdrExec(buildWorktreeOpenArgs(name, cwd, checkoutPath));
	} else {
		output = herdrExec(buildWorktreeCreateArgs(name, cwd, branch, base));
	}
	try {
