...
describe("herdr.ts", () => {
	...
	describe("herdr response parsing", () => {
		...
		it("extracts the surface from a worktree open response", () => {
			const output = JSON.stringify({
				result: {
					type: "worktree_opened",
					already_open: false,
					workspace: { workspace_id: "w1M" },
					root_pane: { pane_id: "w1M:p1" },
					worktree: {
						path: "/repo-wt/probe-x",
						branch: "probe-x",
						open_workspace_id: "w1M",
					},
				},
			});
		...
		it("builds wt create args headlessly", () => {
			assert.deepEqual(__herdrTest__.buildWtCreateArgs("probe-x"), [
				"switch",
				"--create",
				"--no-cd",
				"--format",
			...
			assert.deepEqual(__herdrTest__.buildWtCreateArgs("probe-x", "main"), [
				"switch",
				"--create",
				"--no-cd",
				"--format",
		...
		it("parses the path from wt switch json output", () => {
			const output = [
				'{"action":"created","branch":"probe-x","path":"/repo-wt/probe-x","created_branch":true,"base_branch":"main"}',
				"✓ Created branch probe-x from main and worktree @ /repo-wt/probe-x",
			].join("\n");
			assert.equal(
				__herdrTest__.parseWtSwitchOutput(output),
				"/repo-wt/probe-x",
			);
			assert.throws(
				() => __herdrTest__.parseWtSwitchOutput("no json here"),
				/Unexpected wt switch output/,
			);
		});

		it("builds herdr worktree open args for an existing checkout", () => {
			assert.deepEqual(
				__herdrTest__.buildWorktreeOpenArgs(
					"probe subagent",
					"/repo",
					"/repo-wt/probe-x",
				),
