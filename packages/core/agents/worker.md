...
```text
/skill:review-claims <change-name>
...
Review findings must be grounded and actionable: location, evidence, problem, impact, severity, and the smallest concrete fix. Claim verification uses `Verified`, `Weakened`, and `Falsified` rows with concrete evidence.
