...
describe("review-gated schema assets", () => {
	...
	it("defines focused post-implementation specialist reviews without generic claim/security/performance gates", () => {
		const schema = loadSchema();
		const artifacts = artifactById(schema);

		expect(artifacts.has("review-claims")).toBe(false);
