#!/usr/bin/env node
/**
 * E2E integration test for pi-openspec-workflow.
 *
 ...
 *   E2E_PACKAGE_SOURCE=/absolute/path/to/packages/core
 */
...
const ROOT = mkdtempSync(`${tmpdir()}/pi-openspec-workflow-e2e-`);
...
run("install pi-openspec-workflow", "pi", ["install", "-l", PACKAGE_SOURCE], { cwd: PROJECT });
const list = run("list installed packages", "pi", ["list"], { cwd: PROJECT }).combined;
assertIncludes(list, "pi-openspec-workflow", "install/list");
...
assertIncludes(guidance, "review-plan", "workflow guidance");
...
const reviewPlan = piPrompt(
	"review-plan skill invocation",
	"/skill:review-plan sample-change\nSummarize the exact lower-level review skills this workflow invokes. Do not run tools.",
);
assertIncludes(reviewPlan, "review-proposal", "review-plan");
assertIncludes(reviewPlan, "review-specs", "review-plan");
assertIncludes(reviewPlan, "review-design", "review-plan");
assertIncludes(reviewPlan, "review-tasks", "review-plan");
assertNotIncludes(reviewPlan, "git commit", "review-plan");

const reviewImplementation = piPrompt(
	"review-implementation skill invocation",
	"/skill:review-implementation sample-change\nSummarize the exact lower-level review skills this workflow invokes. Do not run tools.",
);
assertIncludes(reviewImplementation, "review-claims", "review-implementation");
...
console.error("\n[E2E] pi-openspec-workflow real Pi workflow test passed");
