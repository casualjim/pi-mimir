 ...
 *
 * Enhances read, ls, find, and grep with:
 ...
 *
 * The bash tool is owned by pi-heimdall (sandboxed execute); it attaches
 * pi-pretty's bash renderers from ./tools/bash.js so output stays styled.
 * pi-pretty never registers or substitutes bash itself.
 */
...
export default async function piPrettyExtension(pi: ExtensionAPI, deps?: PiPrettyDeps): Promise<void> {
	...
	// SAFETY: the host SDK re-exports CustomEditor only in TUI-capable builds;
	// presence is feature-detected (typeof check) before any use.
	...
	const installPromptEditor = (ctx: ExtensionContext): void => {
		...
		// SAFETY: EditorUiCompatibility members are optional and each use is guarded
		// by a typeof check; older hosts simply omit them.
	...
	const startThinkingShimmer = (ctx: ExtensionContext): void => {
		...
		try {
			...
			const resumeMs = activeTs === undefined ? 0 : (thinkingElapsedMs.get(activeTs) ?? 0);
	...
	pi.on("session_shutdown", async (_event: unknown, ctx?: ExtensionContext) => {
		if (promptEditorInstalled) {
			if (ctx) {
				// SAFETY: optional host hooks; typeof-guarded before the call below.
				const editorUi = ctx.ui as unknown as EditorUiCompatibility;
