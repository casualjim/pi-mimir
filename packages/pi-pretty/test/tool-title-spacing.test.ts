...
describe("tool title/result spacing (blank rows around titles)", () => {
	...
	it("bash collapsed: summary moves into the title row", () => {
		const r = createBashRenderers(MockText);
		const state: Record<string, unknown> = {};
		const call = r.renderCall({ command: "echo out" }, mockTheme, ctx(false, state));
		const result = r.renderResult(
			{ content: [{ type: "text", text: "out" }], details: { _type: "bashResult", text: "out", exitCode: 0, command: "echo out" } },
			{},
			mockTheme,
			ctx(false, state),
		);
		const rows = stackedRows(call as MockText, result as MockText);
	...
	it("bash expanded: title padding does not remove internal info/body spacing", () => {
		const r = createBashRenderers(MockText);
		const state: Record<string, unknown> = {};
		const call = r.renderCall({ command: "echo out" }, mockTheme, ctx(true, state));
		const result = r.renderResult(
			{ content: [{ type: "text", text: "out" }], details: { _type: "bashResult", text: "out", exitCode: 0, command: "echo out" } },
			{},
			mockTheme,
