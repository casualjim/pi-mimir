...
const jitiCandidates = [
  join(dirname(fileURLToPath(sdk)), '../node_modules/jiti/lib/jiti-static.mjs'),
  join(root, 'node_modules/jiti/lib/jiti-static.mjs'),
];
const jitiPath = jitiCandidates.find((candidate) => { try { return statSync(candidate).isFile(); } catch { return false; } });
if (!jitiPath) throw new Error('jiti not found: ' + jitiCandidates.join(', '));
