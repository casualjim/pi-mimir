...
describe("piPrettyExtension integration", () => {
	it.each(["src/index.ts"])("registers pretty tools from managed %s without a local SDK", async (entry) => {
		const tempRoot = mkdtempSync(join(tmpdir(), "pi-pretty-managed-package-"));
		const extensionRoot = join(tempRoot, "node_modules", "@casualjim", "pi-pretty");
	...
	beforeEach(() => {
		...
		for (const fn of [findExec, grepExec, readExec, lsExec]) fn.mockReset();
