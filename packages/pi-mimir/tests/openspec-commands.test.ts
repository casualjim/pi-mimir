...
describe("/openspec:* commands", () => {
	...
	it("runs openspec init --tools pi, then configures schema and seeds assets", async () => {
		...
		expect(harness.execCalls.some((call) => call.cmd === "pi" && call.args.join(" ") === "install npm:pi-mcp-adapter")).toBe(true);
		expect(harness.execCalls.some((call) => call.args.join(" ").includes("@juicesharp"))).toBe(false);
		expect(harness.notifications.at(-1)?.message).toContain("codebase-memory MCP is required");
		expect(harness.notifications.at(-1)?.level).toBe("warning");
	});

	it("reports a warning when pi-mcp-adapter install throws", async () => {
		const harness = createHarness({
			execStubs: {
				"pi install npm:pi-mcp-adapter": async () => {
		...
		expect(harness.notifications.at(-1)?.message).toContain("pi install npm:pi-mcp-adapter failed: network unavailable");
		expect(harness.notifications.at(-1)?.level).toBe("warning");
	});
