...
const msgMissingCodebaseMemory = (pluginInstalled: boolean, missingTools: string[]) =>
	pluginInstalled
		...
		: `codebase-memory support is required for the full pi-openspec architecture-memory-first workflow. Install it with: ${CODEBASE_MEMORY_INSTALL_COMMAND}. Expected tools: ${EXPECTED_CODEBASE_MEMORY_TOOLS.join(", ")}. Degraded discovery is in effect until the plugin is installed and active.`;
const msgWorkflowOverlap = (list: string) =>
	`Known workflow plugin overlap detected (${list}). pi-openspec complements OpenSpec and offers optional composed plan/implement workflows; it does not treat workflow skill names or generated OpenSpec skills as conflicts.`;
...
const WORKFLOW_GUIDANCE = [
	...
	"",
	"Package entrypoints: `plan` runs the full planning workflow by composing OpenSpec proposal/spec/design/task behavior with one planning review; `implement` runs apply execution and verification; `review-plan` runs standalone planning review. Implementation review is explicit, separate from `implement`, and provided by standalone `@casualjim/pi-review` when installed. Generated `/opsx:*` or `openspec-*` skills may coexist and may be called internally by these workflows. Do not add commit, push, PR creation, or finishing-branch behavior as workflow steps.",
	"",
