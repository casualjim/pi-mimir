...
- `review-plan` — separate artifact review workflow that orchestrates `reviews/proposal.md`, `reviews/specs.md`, `reviews/design.md`, and `reviews/tasks.md`.
- `review-implementation` — separate implementation review workflow that orchestrates `reviews/claims.md`, `reviews/architecture.md`, `reviews/tests.md`, `reviews/performance.md`, and `reviews/security.md`.
...
If codebase-memory is unavailable or stale, report degraded discovery, fall back to exact reads or shell inspection, and do not claim architecture-aware discovery.

Artifact review gates run after planning artifacts are complete, in order: proposal, specs, design, tasks. The same gate set can be run independently with `review-plan`.
