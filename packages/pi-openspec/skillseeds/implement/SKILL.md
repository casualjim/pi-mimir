...
8. If the user explicitly asks for implementation review, or if you need a separate review decision after verification, hand off to the standalone `@casualjim/pi-review` skill when available:

```text
/skill:review-implementation <change-name>
```

If `review-implementation` is unavailable, tell the user to install `@casualjim/pi-review` or proceed with verification-only results. Review findings are separate from apply completion and may be returned inline or saved only if the caller explicitly asks.
