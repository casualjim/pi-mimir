...
describe("skill frontmatter", () => {
	...
	it("uses narrow OpenSpec-specific public workflow descriptions", () => {
		...
		expect(descriptionOf(readSkillSeed("review-plan"))).toContain("review");
		expect(descriptionOf(readSkillSeed("review-plan"))).toContain("Use only when");
	});
	...
	it("keeps planning review scoped to explicit OpenSpec review requests", () => {
		const text = readSkillSeed("review-plan");
