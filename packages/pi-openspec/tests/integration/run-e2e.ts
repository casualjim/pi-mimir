#!/usr/bin/env node
/**
 * E2E integration test for pi-openspec.
 *
 ...
 *   E2E_PACKAGE_SOURCE=/absolute/path/to/packages/pi-openspec
 */
...
const ROOT = mkdtempSync(`${tmpdir()}/pi-openspec-e2e-`);
...
run("install pi-openspec", "pi", ["install", "-l", PACKAGE_SOURCE], { cwd: PROJECT });
const list = run("list installed packages", "pi", ["list"], { cwd: PROJECT }).combined;
assertIncludes(list, "pi-openspec", "install/list");
...
console.error("\n[E2E] pi-openspec real Pi workflow test passed");
