...
import {
	...
	createBackgroundTaskLogFile,
	createBackgroundTaskShellEnv,
	deleteBackgroundTaskLogFile,
	formatDuration,
	formatRelativeTime,
...
export default function registerBackgroundTasksExtension(pi: ExtensionAPI): void {
	...
	let preflightState: HeimdallPreflightState = { secretGuard: { secretKeys: [], secretValues: {}, keyPattern: null } };
	...
	const getSortedTasks = (): ManagedTask[] =>
		[...tasks.values()].sort((left, right) => right.startedAt - left.startedAt);
	...
	const getVisibleOutput = (task: ManagedTask): string => redactBackgroundOutput(getTaskOutput(task), disabledSet, preflightState);
	...
	const cleanupTaskLog = (task: ManagedTask) => {
		deleteBackgroundTaskLogFile(task.logFile);
	};
	...
	const sendTaskEvent = (
		...
		const details: BackgroundTaskEventDetails = {
			...
			outputTail: tailText(getVisibleOutput(task), BG_OUTPUT_ALERT_MAX_CHARS),
			task: taskSnapshot(task),
		};
		const status = summarizeTaskStatus(task.status, task.exitCode);
		const wakeReason = eventType === "exit" ? "finished" : "emitted matching output";
		const preview = eventType === "output" ? (options.newOutputTail ?? details.outputTail) : details.outputTail;
		const lines = [
			`Background task ${task.id} ${wakeReason}.`,
			`PID: ${task.pid}`,
			`Status: ${status}`,
			`Command: ${task.command}`,
			`Log: ${task.logFile}`,
			"Recent output:",
			preview.trim().length > 0 ? preview : "(no output yet)",
		];
		pi.sendMessage(
			{
	...
	const scheduleOutputReaction = (task: ManagedTask) => {
		...
		task.outputTimer = setTimeout(() => {
			...
			const unseenOutput = fullOutput.slice(task.lastAlertLength);
			if (!unseenOutput.trim()) {
				task.lastAlertLength = fullOutput.length;
				return;
			}

			if (task.matcher && !(task.matcher(unseenOutput) || task.matcher(fullOutput))) {
				return;
			}
			...
			sendTaskEvent("output", task, {
				...
				newOutputTail: tailText(redactBackgroundOutput(unseenOutput, disabledSet, preflightState), BG_OUTPUT_ALERT_MAX_CHARS),
			});
			refreshUi();
		}, BG_OUTPUT_SETTLE_MS);
	...
	const stopTask = (task: ManagedTask | null): { ok: boolean; message: string } => {
		...
		terminateSandboxProcessGroup(task.child, "SIGTERM");
		refreshUi();
	...
	const spawnTask = async (options: SpawnTaskOptions): Promise<ManagedTask> => {
		if (!sandboxConfig) {
			throw new Error("Sandboxed background tasks require sandbox.enabled=true in Heimdall config.");
		}
		if ((pi.getFlag("no-sandbox") as boolean | undefined) === true) {
			throw new Error("Sandboxed background tasks are disabled for this session via --no-sandbox.");
		}

		const command = options.command.trim();
		const cwd = options.cwd?.trim() || activeCtx?.cwd || process.cwd();
		const blockedReason = getBackgroundCommandBlockReason(command, config, disabledSet, preflightState);
		if (blockedReason) {
			throw new Error(blockedReason);
		}

		const binaryResolution = resolveHeimdallSandboxBinary(sandboxConfig.binaryPath);
		if (!binaryResolution.found && !sandboxConfig.binaryPath) {
			throw new Error(MISSING_BINARY_MESSAGE);
		}

		const id = `bg-${++taskCounter}`;
		const logFile = createBackgroundTaskLogFile(Date.now(), id, getAgentDir());
		...
		try {
			const { child, policyJson } = await launchSandboxProcess(sandboxConfig, command, {
				binaryPath: binaryResolution.binaryPath,
				cwd,
				env: createBackgroundTaskShellEnv(),
			});
			...
			deleteBackgroundTaskLogFile(logFile);
			throw error;
		}
	};
		...
		await ctx.ui.custom(
			(tui, theme, _keybindings, done) => {
				...
				const getOutputLines = (task: ManagedTask | null): string[] => {
					const text = tailText(task ? getVisibleOutput(task) : "", BG_LOG_TAIL_MAX_CHARS).trim();
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (Array.isArray(config.disabled)) {
			for (const disabled of config.disabled) {
				disabledSet.add(disabled);
			}
		}

		sandboxConfig = null;
		const noSandbox = (pi.getFlag("no-sandbox") as boolean | undefined) === true;
		if (!noSandbox) {
			const normalized = normalizeSandboxConfig(
				config.sandbox as SandboxConfig | undefined,
				projectConfigPath,
			);
			if (normalized.enabled) {
				sandboxConfig = normalized;
			}
		}
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params): Promise<AgentToolResult<unknown>> {
			...
			if (params.pid === undefined) {
				return makeToolResult("PID is required for bg_status log/stop.", { isError: true });
			}
			...
			if (params.action === "log") {
				return makeToolResult(tailText(getVisibleOutput(task), BG_LOG_TAIL_MAX_CHARS) || "(empty)");
			}
	...
	pi.registerTool({
		description:
			"Spawn, inspect, and stop sandboxed background shell tasks. Tasks keep running after the tool returns, append output to a private Heimdall log file, and can wake the agent when new output arrives or when the task exits. Background tasks expire after 10 minutes by default to prevent indefinite runs.",
		async execute(_toolCallId, params): Promise<AgentToolResult<unknown>> {
			const { action } = params;
			...
			if (action === "spawn") {
				...
				try {
					...
					return makeToolResult(error instanceof Error ? error.message : String(error), { isError: true });
				}
			}
	...
	pi.registerCommand(BG_COMMAND, {
		...
		handler: async (args, ctx) => {
			...
			if (trimmed.startsWith("run ")) {
				...
				try {
					const task = await spawnTask({ command, cwd: ctx.cwd });
