...
export const BG_MESSAGE_TYPE = "heimdall-bg-task:event";
export const BG_WIDGET_KEY = "heimdall-bg-task";
...
export const BG_INSTALL_SYMBOL = Symbol.for("pi-heimdall.background-tasks.installed");
...
export function createBackgroundTaskShellEnv(
...
export function getBackgroundTaskLogDir(agentDir: string = getAgentDir()): string {
	const dir = join(agentDir, "heimdall", "background-tasks");
	mkdirSync(dir, { recursive: true, mode: 0o700 });
	chmodSync(dir, 0o700);
	return dir;
}

export function createBackgroundTaskLogFile(
	now: number = Date.now(),
	id: string = "bg",
	agentDir: string = getAgentDir(),
): string {
	const dir = getBackgroundTaskLogDir(agentDir);
	const suffix = randomUUID().replaceAll("-", "");
	const path = join(dir, `heimdall-bg-${id}-${now}-${suffix}.log`);
	writeFileSync(path, "", { mode: 0o600 });
	chmodSync(path, 0o600);
	return path;
}

export function deleteBackgroundTaskLogFile(path: string): void {
	try {
