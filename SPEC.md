...
- file: `.pi/heimdall.jsonc` → `commandPolicies[]` policy `bare: true` ! enforce matched command run bare on output side: ⊥ pipe-after (`|`) & output redirect (`>`/`>>`/`>&`/`>|`/`&>`/`&>>`); pipe-input & input redirect allowed.
